...
def load_state_polls():  # produces the csv file polls_aggregated
    df = load_polls()
    ...
    states = list(np.unique(polls['state'].values))
    states.remove('ME-1')
    states.remove('ME-2')
    states.remove('NE-1')
    states.remove('NE-2')
    ...
    for year_election in years_election:
        year_polls = polls[polls['cycle'] == year_election]
        for state in states:
            polls_state = year_polls[year_polls['state'] == state]
            mask_interesting_months = polls_state['modeldate'].apply(
                lambda x: True if (
                            int(x.split('/')[0]) > 7) else False)  # select the polls of election year, from June to October
            interesting_months_poll = polls_state[mask_interesting_months]
            try:
...
def select_candidates(polls_1):  # this function only retains the two best candidates for every election year, being R vs D
    ...
    polls_1.drop(polls_1.columns[0], axis=1, inplace=True)
    for year in years_election:
        polls_year = polls_1[polls_1['cycle'] == year]
        candidates = np.unique(polls_year['candidate_name'].astype('str').values)
...
# todo: fix the bug in the way we get the polls for candidates. Sometimes one of two candidates is being lost (ex Alabama 1968)
